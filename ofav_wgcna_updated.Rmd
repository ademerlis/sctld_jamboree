...
mod = model.matrix(~ Correct_treatment + Experiment, data=ofav_metadata) #full model with batch effect in it
mod0 = model.matrix(~1,data=ofav_metadata) #null model

batch = ofav_metadata$Experiment
modcombat = model.matrix(~ Correct_treatment, data=ofav_metadata) #for model matrix for combat, don't include batch effect in formula
combat_expressiondata = ComBat(dat=cccall_ofav, batch=batch, mod=modcombat)
pValuesComBat = f.pvalue(combat_expressiondata,mod,mod0)
qValuesComBat = p.adjust(pValuesComBat,method="BH")
```
Found 5 genes with uniform expression within a single batch (all zeros); these will not be adjusted for batch.

But, for the PCAs, the limma "removeBatchEffect" function was used, so to keep the methods consistent across the analysis, I will use that same package for the WGCNA analysis.
...
tophub %>% 
  rename(Locus = Gene.ID) -> tophub
tophub %>% rownames_to_column(var="module") ->tophub

ofav_annot_DEG <- read.csv("ofav_DEG_annotated_updated.csv") #this table is linking gene IDs to protein IDs and protein names. the ID matches from the NCBI refseq version of the Ofav genome, created through their automatic annotation pipeline

ofav_annot_DEG %>% 
  dplyr::select(X, Protein.Name) %>% 
  rename(Locus = X) %>% 
  right_join(tophub) %>% 
  as.data.frame() %>% 
 select(!Locus) -> tophub_annotated
...
module_annotation <- ofav_annot_DEG[ofav_annot_DEG$X %in% modGeneIDs, ]
